...
export function GamePageLayout() {
  ...
  async function handleSolved() {
    ...
    try {
      const score = await finishPlayerRound({
        roomId, roundId, uid, name: playerName,
        status: 'solved', bestRank: 1,
        guessCount: localGuesses.length, usedHints, hintPenalty: 0,
        startedAtMs, finishOrder: publicResults.length + 1,
      })
      setRoundScore(score)
      setShowCelebration(true)
    } catch (e) { console.error('finishPlayerRound error', e) }
  }
  ...
  async function handleSurrender() {
    ...
    const bestRank = localGuesses.reduce<number | null>(
      (min, g) => g.rank === null ? min : (min === null || g.rank < min ? g.rank : min), null
    )
    try {
      await finishPlayerRound({
        roomId, roundId, uid, name: playerName,
        status: 'surrendered', bestRank,
        guessCount: localGuesses.length, usedHints, hintPenalty: 0,
        startedAtMs, finishOrder: publicResults.length + 1,
      })
    } catch (e) { console.error('finishPlayerRound error', e) }
    ...
    <div className="min-h-screen p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[3fr_2fr] gap-4 max-w-5xl mx-auto">
        ...
        <div className="flex flex-col gap-3">
          ...
          />
          {isPlaying && (
            <GuessInputForm roundSalt={roundSalt} roomId={roomId} roundId={roundId} disabled={false} onSolved={handleSolved} />
          )}
          ...
          {showSurrenderConfirm && isPlaying && (
            <div className="rounded-xl border border-destructive/40 bg-destructive/10 p-4 flex flex-col gap-3">
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowSurrenderConfirm(false)} className="flex-1 rounded-md border border-border px-3 py-1.5 text-sm hover:bg-muted/20">Huỷ</button>
                <button onClick={handleSurrender} className="flex-1 rounded-md bg-destructive/80 px-3 py-1.5 text-sm text-white hover:bg-destructive">Bỏ cuộc</button>
              </div>
            </div>
          )}
        ...
        <div className="hidden md:flex flex-col gap-3">
          <RealtimeRoundResultsBoard roomId={roomId} roundId={roundId} activePlayers={activePlayers} />
