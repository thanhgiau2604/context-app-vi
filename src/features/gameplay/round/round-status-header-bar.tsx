...
export function RoundStatusHeaderBar({
  roundNumber, startedAtMs, onHint, onSurrender, hintDisabled, surrenderDisabled, usedHints
}: Props) {
  const { bestRank, localGuesses } = useGameStore()
  const [elapsedSec, setElapsedSec] = useState(0)

  useEffect(() => {
    const id = setInterval(() => setElapsedSec(Math.floor((Date.now() - startedAtMs) / 1000)), 1000)
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border bg-game-surface-1 px-4 py-3">
      <Badge variant="outline" className="font-mono">Round #{roundNumber}</Badge>
      ...
      <span className="ml-auto font-mono text-sm tabular-nums text-muted-foreground">{mm}:{ss}</span>
      <button
        onClick={onHint}
