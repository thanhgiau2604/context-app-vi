...
import { createGame, type CreateGameProgress, type CreateGameResult } from './create-game-orchestration-service'
...
export function CreateGameModalDialog({ open, onOpenChange, roomId, adminUid, onDone }: Props) {
  ...
  async function handleGenerate() {
    if (!geminiApiKey) { toast.error('Chưa cài API key Gemini. Vào Cài đặt trước.'); return }
    setStep('generating')
    setResult(null)
    try {
      const r = await createGame(
        { roomId, adminUid, apiKey: geminiApiKey, model: geminiModel, topic: topic || undefined, difficulty },
        handleProgress
      )
      setResult(r)
      setStep('preview')
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <Tabs defaultValue="gemini">
          <TabsList className="w-full">
            ...
            <TabsTrigger value="json" className="flex-1">Import JSON</TabsTrigger>
          </TabsList>

          <TabsContent value="gemini" className="flex flex-col gap-4 pt-2">
            <div className="flex flex-col gap-2">
              <Label>Chủ đề (tuỳ chọn)</Label>
              <Input placeholder="VD: Thể thao, Ẩm thực, Thiên nhiên…" value={topic} onChange={(e) => setTopic(e.target.value)} disabled={step === 'generating'} />
            </div>
            <div className="flex flex-col gap-2">
              <Label>Độ khó</Label>
              <Select value={difficulty} onValueChange={(v) => setDifficulty(v as typeof difficulty)} disabled={step === 'generating'}>
            ...
            <Button onClick={handleGenerate} disabled={step === 'generating' || step === 'confirming'}>
              {step === 'generating'
                ? <><Loader2 size={16} className="mr-2 animate-spin" />{progress?.message ?? 'Đang tạo…'}</>
                : <><Sparkles size={16} className="mr-2" />Tạo với Gemini</>}
            </Button>
            ...
            {result && step === 'preview' && (
              <div className="flex flex-col gap-3 rounded-lg border border-border bg-muted/20 p-4">
                ...
                <Button onClick={handleConfirmAndStart} className="mt-1">
                  <CheckCircle size={16} className="mr-2" />Xác nhận &amp; Bắt đầu round
                </Button>
              </div>
            )}
          </TabsContent>
          ...
          <TabsContent value="json" className="flex flex-col gap-4 pt-2">
            ...
            {pendingJson && (
              <Button onClick={handleImportConfirm} disabled={step === 'confirming'}>
                {step === 'confirming'
                  ? <><Loader2 size={16} className="mr-2 animate-spin" />{progress?.message}</>
                  : <><CheckCircle size={16} className="mr-2" />Xác nhận &amp; Tạo game</>}
              </Button>
            )}
          </TabsContent>
        </Tabs>
